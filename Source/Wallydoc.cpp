...
{	
	...
	else
	{
		...
		strcpy (SinHeader.name, szName);
		TempName.ReleaseBuffer();
	
		TempName = TempName.Left (31);
		szName = TempName.GetBuffer (TempName.GetLength());
		strcpy (Q2Header.name, szName);
		TempName.ReleaseBuffer();

		TempName = TempName.Left (15);
		szName = TempName.GetBuffer (TempName.GetLength());
		strcpy (Q1Header.name, szName);
		TempName.ReleaseBuffer();		

		TempName = TempName.Left (31);
		szName = TempName.GetBuffer (TempName.GetLength());
		strcpy (M8Header.name, szName);
		TempName.ReleaseBuffer();	
	}
...
{		
	...
	case FILE_TYPE_HERETIC2:
		...
		if (TexInfoDlg.DoModal() == IDOK)
		{
			...
			strcpy (M8Header.name, TempHeader.name);
			strcpy (M8Header.animname, TempHeader.animname);
		}
		break;
