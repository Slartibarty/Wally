...
//		sprintf (sbText, "Size:%3d x %3d", w, h);
		...
		{
		case 8:
			strcpy( szMsg, "%d x %d, 256 color");
			break;
		case 24:
		case 32:
			sprintf (szMsg, "%%d x %%d, %d-bit", iBits);
			break;
		default:
			strcpy( szMsg, "%d x %d, <unknown bit depth>");
			break;
		}

		sprintf (szText, szMsg, w, h);
...
//			sprintf (sbText, "Zoom:1:%d", -m_iZoomValue);
			...
			if (m_iZoomValue >= 1)
				sprintf( szText, "Zoom: %d%%", (int )(m_dfZoom * 100.0));
			else
				sprintf( szText, "Zoom: %1.3f%%", m_dfZoom * 100.0);
...
//		sprintf( szZoom, "Zoom:1:%d", -m_iZoomValue);
	...
	if (m_iZoomValue >= 1)
		sprintf( szZoom, "Zoom: %d%%", (int )(m_dfZoom * 100.0));
	else
		sprintf( szZoom, "Zoom: %1.3f%%", m_dfZoom * 100.0);
