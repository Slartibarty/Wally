...
void DebugOut (char *text, bool overwrite)
{
	...
	FILE *wp;
	if (overwrite)
		wp = fopen ("c:\\temp\\wallyout.txt","w");
	else
		wp = fopen ("c:\\temp\\wallyout.txt","a");
...
CString GetExtension( CString strFileName)
{
	...
	_splitpath( strFileName, szDrive, szDir, szFileName, szExt);
...
CString BrowseForFolder (LPCTSTR szTitle, LPCTSTR szStartDirectory /* = NULL */)
{
	...
	if (szStartDirectory)
	{
		...
		strcpy (szName, strStartDirectory.GetBuffer(strStartDirectory.GetLength()));
	}
